...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased font-sans {{ $t['bg_body'] }} {{ $t['text_main'] }}">
        ...
        <div class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] flex space-x-2 bg-gray-900/90 backdrop-blur-xl p-1.5 rounded-full shadow-2xl border border-white/10 w-[calc(100%-2rem)] max-w-sm lg:w-auto">
            <a href="{{ route('sales-pages.index') }}" class="flex-1 lg:flex-none bg-white/5 text-white px-6 py-2.5 rounded-full shadow-lg hover:bg-white/10 transition flex items-center justify-center text-xs font-bold uppercase tracking-widest leading-none">
                Dashboard
            </a>
            <a href="{{ route('sales-pages.export', $salesPage) }}" class="flex-1 lg:flex-none bg-green-600 text-white px-6 py-2.5 rounded-full shadow-lg hover:bg-green-700 transition flex items-center justify-center text-xs font-bold uppercase tracking-widest leading-none whitespace-nowrap">
                Export HTML
            </a>
        </div>
        ...
        <nav class="{{ $t['bg_card'] }} border-b {{ $t['border'] }} py-4 px-6 fixed w-full z-50 top-0 shadow-sm {{ $isFuture ? 'bg-opacity-80 backdrop-blur-xl' : '' }}">
            <div class="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="font-bold text-xl {{ $t['primary_text'] }} uppercase tracking-tighter text-center sm:text-left break-words max-w-full">{{ $salesPage->product->name }}</div>
                <div class="flex items-center space-x-6">
                    ...
                    <a href="#pricing" class="{{ $t['primary'] }} text-{{ $isFuture ? 'black' : 'white' }} px-6 py-2.5 rounded-full font-black {{ $t['primary_hover'] }} transition-all text-[10px] uppercase tracking-widest whitespace-nowrap">Mulai Sekarang</a>
                </div>
            </div>
        </nav>

        <!-- Hero Section -->
        <section class="pt-40 lg:pt-48 pb-20 lg:pb-32 px-4 lg:px-6 {{ $t['bg_hero'] }} relative overflow-hidden">
            @if($isFuture)
            ...
            <div class="max-w-5xl mx-auto text-center relative z-10">
                <h1 class="text-4xl md:text-7xl font-black tracking-tighter mb-8 leading-[0.95] {{ $isFuture ? 'bg-clip-text text-transparent bg-gradient-to-b from-white to-slate-500' : '' }}">
        ...
        <!-- Problem & Solution -->
        <section class="py-16 lg:py-32 px-4 lg:px-6 relative">
            <div class="max-w-5xl mx-auto">
                <div class="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
                    <div class="{{ $salesPage->theme === 'luxury' || $isFuture ? 'bg-red-950/10 border-red-900/30 text-red-200' : 'bg-red-50 border-red-100 text-red-700' }} p-8 lg:p-10 rounded-[2.5rem] border relative overflow-hidden group">
                    ...
                    <div class="{{ $salesPage->theme === 'luxury' || $isFuture ? 'bg-cyan-950/10 border-cyan-900/30 text-cyan-100' : 'bg-green-50 border-green-100 text-green-700' }} p-8 lg:p-10 rounded-[2.5rem] border relative overflow-hidden group">
                        <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-150 transition-transform">
        ...
        <!-- Benefits -->
        <section id="benefits" class="py-16 lg:py-32 px-4 lg:px-6 bg-opacity-50 {{ $isFuture ? 'bg-[#0d0d10]' : '' }}">
            <div class="max-w-6xl mx-auto">
                <h2 class="text-3xl md:text-5xl font-black text-center mb-12 lg:mb-20 uppercase tracking-tighter">Why Choose Us?</h2>
                <div class="grid md:grid-cols-3 gap-6 lg:gap-10">
                    @foreach($salesPage->content['benefits'] as $benefit)
                    <div class="{{ $t['bg_card'] }} p-8 lg:p-10 rounded-[2rem] shadow-sm border {{ $t['border'] }} hover:border-{{ str_replace('text-', '', $t['accent_text']) }} transition-all group">
        ...
        <section id="features" class="py-16 lg:py-32 px-4 lg:px-6">
            <div class="max-w-6xl mx-auto">
                <h2 class="text-3xl md:text-5xl font-black text-center mb-16 lg:mb-24 uppercase tracking-tighter">Core Features</h2>
                <div class="space-y-24 lg:space-y-32">
                    @foreach($salesPage->content['detailed_features'] as $index => $feature)
                    <div class="flex flex-col {{ $index % 2 == 0 ? 'md:flex-row' : 'md:flex-row-reverse' }} items-center gap-12 lg:gap-20">
                        <div class="flex-1">
                        ...
                        <div class="flex-1 w-full aspect-video {{ $isFuture ? 'bg-gradient-to-br from-slate-800 to-slate-900 border-slate-700' : 'bg-gray-100' }} rounded-[2.5rem] lg:rounded-[3rem] flex items-center justify-center text-slate-500 italic border-4 {{ $t['border'] }} shadow-2xl">
        ...
        <section id="pricing" class="py-24 lg:py-40 px-4 lg:px-6 relative overflow-hidden">
            ...
            @endif
            <div class="max-w-2xl mx-auto {{ $t['bg_card'] }} rounded-[2.5rem] lg:rounded-[3rem] shadow-2xl overflow-hidden border-2 border-{{ str_replace('bg-', '', $t['primary']) }} relative z-10 transform hover:scale-[1.02] transition-transform duration-500">
                ...
                <div class="p-8 lg:p-16 text-center">
                    ...
                    <a href="#" class="block w-full {{ $t['primary'] }} text-{{ $isFuture ? 'black' : 'white' }} py-6 lg:py-8 rounded-[2rem] text-2xl font-black {{ $t['primary_hover'] }} transition-all shadow-xl {{ $t['shadow'] }} uppercase tracking-widest">
                        {{ $salesPage->content['cta_text'] }}
                    </a>
        ...
        <section id="faq" class="py-16 lg:py-32 px-4 lg:px-6">
            <div class="max-w-4xl mx-auto">
                <h2 class="text-3xl md:text-4xl font-black text-center mb-12 lg:mb-20 uppercase tracking-tighter">Common Questions</h2>
        ...
        <footer class="py-20 px-4 lg:px-6 border-t {{ $t['border'] }} text-center text-slate-500">
